        ...
        internal static string EscapeQName(string name)
        {
            ...
            else
            {
                if (colon == 0 || colon == name.Length - 1)
                    throw new ArgumentException(SRFormat.Format(SR.Xml_InvalidNameChars, name), nameof(name));
    ...
    internal class AttributeAccessor : Accessor
    {
        ...
        internal void CheckSpecial()
        {
            ...
            if (colon >= 0)
            {
                ...
                {
                    throw new InvalidOperationException(SRFormat.Format(SR.Xml_InvalidNameChars, Name));
                }
            ...
            for (int i = 0; i < _members.Length; i++)
            {
                if (_members[i].Name == member.Name)
                {
                    if (_members[i].TypeDesc != member.TypeDesc)
                        throw new InvalidOperationException(SRFormat.Format(SR.XmlHiddenMember, parent, member.Name, member.TypeDesc.FullName, this.TypeName, _members[i].Name, _members[i].TypeDesc.FullName));
                    else if (!_members[i].Match(member))
                    {
                        throw new InvalidOperationException(SRFormat.Format(SR.XmlInvalidXmlOverride, parent, member.Name, this.TypeName, _members[i].Name));
                    }
        ...
        internal void SetContentModel(TextAccessor text, bool hasElements)
        {
            ...
            else if (BaseMapping.HasSimpleContent)
            {
                if (text != null || hasElements)
                {
                    ...
                    throw new InvalidOperationException(SRFormat.Format(SR.XmlIllegalSimpleContentExtension, TypeDesc.FullName, BaseMapping.TypeDesc.FullName));
                }
                else
                {
            ...
            {
                throw new InvalidOperationException(SRFormat.Format(SR.XmlIllegalTypedTextAttribute, TypeDesc.FullName, text.Name, text.Mapping.TypeDesc.FullName));
            }
        }
    ...
    internal class SerializableMapping : SpecialMapping
    {
        ...
        internal void SetBaseMapping(SerializableMapping mapping)
        {
            ...
            if (_baseMapping != null)
            {
                ...
                {
                    throw new InvalidOperationException(SRFormat.Format(SR.XmlCircularDerivation, TypeDesc.FullName));
                }
            }
        }
        ...
        internal void CheckDuplicateElement(XmlSchemaElement element, string elementNs)
        {
            ...
            {
                throw new NotImplementedException("Schema != null && Schema.TargetNamespace == elementNs");
            ...
            foreach (XmlSchemaElement e in elements.Values)
            {
                if (e.Name == element.Name && e.QualifiedName.Namespace == elementNs)
                {
                    ...
                    throw new InvalidOperationException(SRFormat.Format(SR.XmlSerializableRootDupName, _getSchemaMethod.DeclaringType.FullName, e.Name, elementNs));
                }
            }
        }
        ...
        private void RetrieveSerializableSchema()
        {
            if (_needSchema)
            {
                ...
                if (_getSchemaMethod != null)
                {
                    ...
                    if (typeInfo != null)
                    {
                        ...
                        else if (typeof(XmlQualifiedName).IsAssignableFrom(_getSchemaMethod.ReturnType))
                        {
                            ...
                            if (_xsiType.IsEmpty)
                            {
                                throw new InvalidOperationException(SRFormat.Format(SR.XmlGetSchemaEmptyTypeName, _type.FullName, _getSchemaMethod.Name));
                            }
                        }
                        else
                        {
                            throw new InvalidOperationException(SRFormat.Format(SR.XmlGetSchemaMethodReturnType, _type.Name, _getSchemaMethod.Name, typeof(XmlSchemaProviderAttribute).Name, typeof(XmlQualifiedName).FullName));
                        }
                    }
                    else
                    {
                    ...
                    //        {
                    //            throw new InvalidOperationException(SRFormat.Format(SR.XmlMissingSchema, _xsiType.Namespace));
                    //        }
                    //        if (srcSchemas.Count > 1)
                    //        {
                    //            throw new InvalidOperationException(SRFormat.Format(SR.XmlGetSchemaInclude, _xsiType.Namespace, _getSchemaMethod.DeclaringType.FullName, _getSchemaMethod.Name));
                    //        }
                    ...
                    //        if (_xsdType == null)
                    //        {
                    //            throw new InvalidOperationException(SRFormat.Format(SR.XmlGetSchemaTypeMissing, _getSchemaMethod.DeclaringType.FullName, _getSchemaMethod.Name, _xsiType.Name, _xsiType.Namespace));
                    //        }
                ...
                else
                {
                    ...
                    if (_schema != null)
                    {
                        if (_schema.Id == null || _schema.Id.Length == 0) throw new InvalidOperationException(SRFormat.Format(SR.XmlSerializableNameMissing1, _type.FullName));
                    }
                }
            }
        }
