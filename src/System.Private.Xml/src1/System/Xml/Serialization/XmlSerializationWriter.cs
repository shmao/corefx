...
namespace SGen.Xml.Serialization
{
    ...
    public abstract class XmlSerializationWriter : XmlSerializationGeneratedCode
    {
        ...
        protected void WriteStartElement(string name, string ns, object o, bool writePrefixed, XmlSerializerNamespaces xmlns)
        {
            ...
            {
                if (_objectsInUse.ContainsKey(o)) throw new InvalidOperationException(SRFormat.Format(SR.XmlCircularReference, o.GetType().FullName));
            ...
            if (_namespaces != null)
            {
                foreach (string alias in _namespaces.Namespaces.Keys)
                {
                    ...
                    {
                        if (alias.Length > 0)
                            throw new InvalidOperationException(SRFormat.Format(SR.XmlInvalidXmlns, alias));
        ...
        protected void WriteEndElement(object o)
        {
            ...
            if (o != null && _objectsInUse != null)
            {
                    ...
                    if (!_objectsInUse.ContainsKey(o)) throw new InvalidOperationException(SRFormat.Format(SR.XmlInternalErrorDetails, "missing stack object of type " + o.GetType().FullName));
#endif
            ...
            if (any)
            {
                if (node is XmlElement && name != null && name.Length > 0)
                {
                    ...
                    if (node.LocalName != name || node.NamespaceURI != ns)
                        throw new InvalidOperationException(SRFormat.Format(SR.XmlElementNameMismatch, node.LocalName, node.NamespaceURI, name, ns));
                }
            }
            else
                _w.WriteStartElement(name, ns);
        ...
        {
            if (typeof(IXmlSerializable).IsAssignableFrom(type)) return new InvalidOperationException(SRFormat.Format(SR.XmlInvalidSerializable, type.FullName));
            TypeDesc typeDesc = new TypeScope().GetTypeDesc(type);
            if (!typeDesc.IsStructLike) return new InvalidOperationException(SRFormat.Format(SR.XmlInvalidUseOfType, type.FullName));
            return new InvalidOperationException(SRFormat.Format(SR.XmlUnxpectedType, type.FullName));
        }

        protected Exception CreateMismatchChoiceException(string value, string elementName, string enumValue)
        {
            // Value of {0} mismatches the type of {1}, you need to set it to {2}.
            return new InvalidOperationException(SRFormat.Format(SR.XmlChoiceMismatchChoiceException, elementName, value, enumValue));
        }

        protected Exception CreateUnknownAnyElementException(string name, string ns)
        {
            return new InvalidOperationException(SRFormat.Format(SR.XmlUnknownAnyElement, name, ns));
        }

        protected Exception CreateInvalidChoiceIdentifierValueException(string type, string identifier)
        {
            return new InvalidOperationException(SRFormat.Format(SR.XmlInvalidChoiceIdentifierValue, type, identifier));
        }
        ...
        protected Exception CreateChoiceIdentifierValueException(string value, string identifier, string name, string ns)
        {
            ...
            return new InvalidOperationException(SRFormat.Format(SR.XmlChoiceIdentifierMismatch, value, identifier, name, ns));
        }

        protected Exception CreateInvalidEnumValueException(object value, string typeName)
        {
            return new InvalidOperationException(SRFormat.Format(SR.XmlUnknownConstant, value, typeName));
        }
        ...
        {
            return new InvalidOperationException(SRFormat.Format(SR.XmlIllegalAnyElement, type.FullName));
        }
...
#if DEBUG
                    // use exception in the place of Debug.Assert to avoid throwing asserts from a server process such as aspnet_ewp.exe
                    if (!typeof(IEnumerable).IsAssignableFrom(type)) throw new InvalidOperationException(SRFormat.Format(SR.XmlInternalErrorDetails, "not array like type " + type.FullName));
#endif
            ...
            if (xmlns != null)
            {
                foreach (KeyValuePair<string, string> entry in xmlns.Namespaces)
                {
                    ...
                    if (_namespaces != null)
                    {
                        ...
                        {
                            throw new InvalidOperationException(SRFormat.Format(SR.XmlDuplicateNs, prefix, ns));
                        }
                    }
...
#if DEBUG
                // use exception in the place of Debug.Assert to avoid throwing asserts from a server process such as aspnet_ewp.exe
                if (methodName == null) throw new InvalidOperationException(SRFormat.Format(SR.XmlInternalErrorMethod, mapping.TypeDesc.Name) + Environment.StackTrace);
#endif
...
#if DEBUG
                        // use exception in the place of Debug.Assert to avoid throwing asserts from a server process such as aspnet_ewp.exe
                        if (defaultValue.GetType() != typeof(string)) throw new InvalidOperationException(SRFormat.Format(SR.XmlInternalErrorDetails, name + " has invalid default type " + defaultValue.GetType().Name));
#endif
...
#if DEBUG
                        // use exception in the place of Debug.Assert to avoid throwing asserts from a server process such as aspnet_ewp.exe
                        if (enumSource == null) throw new InvalidOperationException(SRFormat.Format(SR.XmlInternalErrorDetails, "Can not find " + member.ChoiceIdentifier.MemberName + " in the members mapping."));
#endif
                }
...
#if DEBUG
                    // use exception in the place of Debug.Assert to avoid throwing asserts from a server process such as aspnet_ewp.exe
                    if (methodName == null) throw new InvalidOperationException("derived from " + mapping.TypeDesc.FullName + ", " + SRFormat.Format(SR.XmlInternalErrorMethod, derived.TypeDesc.Name) + Environment.StackTrace);
#endif
        ...
        private void WriteValue(object value)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        throw new InvalidOperationException(SRFormat.Format(SR.XmlUnsupportedDefaultType, type.FullName));
                    }
                }
            }
        }
        ...
        private string FindChoiceEnumValue(ElementAccessor element, EnumMapping choiceMapping, bool useReflection)
        {
            ...
            if (enumValue == null || enumValue.Length == 0)
            {
                if (element.Any && element.Name.Length == 0)
                {
                    ...
                    throw new InvalidOperationException(SRFormat.Format(SR.XmlChoiceMissingAnyValue, choiceMapping.TypeDesc.FullName));
                }
                // Type {0} is missing value for '{1}'.
                throw new InvalidOperationException(SRFormat.Format(SR.XmlChoiceMissingValue, choiceMapping.TypeDesc.FullName, element.Namespace + ":" + element.Name, element.Name, element.Namespace));
            }
            if (!useReflection)
    ...
    internal class ReflectionAwareCodeGen
    {
        ...
        private string WriteMemberInfo(Type type, string escapedName, string typeVariable, string memberName)
        {
            ...
            throw new InvalidOperationException(SRFormat.Format(SR.XmlSerializerUnsupportedType, memberInfos[0].ToString()));
        }
