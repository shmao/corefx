...
namespace SGen.Xml.Serialization
{
    ...
    using SGen.Xml.Serialization.Configuration;
    ...
    public abstract class XmlSerializationReader : XmlSerializationGeneratedCode
    {
        ...
        protected int GetArrayLength(string name, string ns)
        {
            ...
            if (arrayInfo.dimensions != 1) throw new InvalidOperationException(SRFormat.Format(SR.XmlInvalidArrayDimentions, CurrentTag()));
            XmlQualifiedName qname = ToXmlQualifiedName(arrayInfo.qname, false);
            if (qname.Name != name) throw new InvalidOperationException(SRFormat.Format(SR.XmlInvalidArrayTypeName, qname.Name, name, CurrentTag()));
            if (qname.Namespace != ns) throw new InvalidOperationException(SRFormat.Format(SR.XmlInvalidArrayTypeNamespace, qname.Namespace, ns, CurrentTag()));
        ...
        private SoapArrayInfo ParseArrayType(string value)
        {
            if (value == null)
            {
                throw new ArgumentNullException(SRFormat.Format(SR.XmlMissingArrayType, CurrentTag()));
            }

            if (value.Length == 0)
            {
                throw new ArgumentException(SRFormat.Format(SR.XmlEmptyArrayType, CurrentTag()), nameof(value));
            }
            ...
            {
                if (chars[pos] == ',')
                    throw new ArgumentException(SRFormat.Format(SR.XmlInvalidArrayDimentions, CurrentTag()), nameof(value));
                pos--;
            }
            if (pos == -1)
            {
            ...
            if (len > 0)
            {
                ...
                catch (Exception e)
                {
                    ...
                    throw new ArgumentException(SRFormat.Format(SR.XmlInvalidArrayLength, lengthString), nameof(value));
                }
            }
            else
            {
                ...
                if (dimensions[i].Length > 0)
                {
                    ...
                    else
                    {
                        ...
                        catch (Exception e)
                        {
                            ...
                            throw new ArgumentException(SRFormat.Format(SR.XmlInvalidArrayLength, dimensions[i]), "value");
                        }
                    }
                }
            }
        ...
        internal XmlQualifiedName ToXmlQualifiedName(string value, bool decodeName)
        {
            ...
            else
            {
                ...
                if (ns == null)
                {
                    ...
                    throw new InvalidOperationException(SRFormat.Format(SR.XmlUndefinedAlias, prefix));
                }
        ...
        {
            return new InvalidOperationException(SRFormat.Format(SR.XmlUnknownType, type.Name, type.Namespace, CurrentTag()));
        }

        protected Exception CreateReadOnlyCollectionException(string name)
        {
            return new InvalidOperationException(SRFormat.Format(SR.XmlReadOnlyCollection, name));
        }

        protected Exception CreateAbstractTypeException(string name, string ns)
        {
            return new InvalidOperationException(SRFormat.Format(SR.XmlAbstractType, name, ns, CurrentTag()));
        }

        protected Exception CreateInaccessibleConstructorException(string typeName)
        {
            return new InvalidOperationException(SRFormat.Format(SR.XmlConstructorInaccessible, typeName));
        }

        protected Exception CreateCtorHasSecurityException(string typeName)
        {
            return new InvalidOperationException(SRFormat.Format(SR.XmlConstructorHasSecurityAttributes, typeName));
        }

        protected Exception CreateUnknownNodeException()
        {
            return new InvalidOperationException(SRFormat.Format(SR.XmlUnknownNode, CurrentTag()));
        }

        protected Exception CreateUnknownConstantException(string value, Type enumType)
        {
            return new InvalidOperationException(SRFormat.Format(SR.XmlUnknownConstant, value, enumType.Name));
        }
        ...
        {
            if (value == null)
                return new InvalidCastException(SRFormat.Format(SR.XmlInvalidNullCast, type.FullName));
            else if (id == null)
                return new InvalidCastException(SRFormat.Format(SR.XmlInvalidCast, value.GetType().FullName, type.FullName));
            else
                return new InvalidCastException(SRFormat.Format(SR.XmlInvalidCastWithId, value.GetType().FullName, type.FullName, id));
        }
        ...
        {
            return new InvalidOperationException(SRFormat.Format(SR.XmlSerializableBadDerivation, xsdDerived, nsDerived, xsdBase, nsBase, clrDerived, clrBase));
        }

        protected Exception CreateMissingIXmlSerializableType(string name, string ns, string clrType)
        {
            return new InvalidOperationException(SRFormat.Format(SR.XmlSerializableMissingClrType, name, ns, typeof(XmlIncludeAttribute).Name, clrType));
        ...
        protected bool ReadReference(out string fixupReference)
        {
            ...
            if (!_soap12)
            {
                ...
                if (!href.StartsWith("#", StringComparison.Ordinal)) throw new InvalidOperationException(SRFormat.Format(SR.XmlMissingHref, href));
        ...
        protected object GetTarget(string id)
        {
            ...
            if (target == null)
            {
                throw new InvalidOperationException(SRFormat.Format(SR.XmlInvalidHref, id));
            }
            Referenced(target);
            return target;
        }
        ...
        protected void FixupArrayRefs(object fixup)
        {
            ...
            for (int i = 0; i < array.Length; i++)
            {
                ...
                {
                    throw new InvalidOperationException(SRFormat.Format(SR.XmlInvalidArrayRef, id, o.GetType().FullName, i.ToString(CultureInfo.InvariantCulture)));
                }
            }
        }
        ...
        private object ReadArray(string typeName, string typeNs)
        {
            ...
            if (elementType.IsValueType)
            {
                ...
                {
                    throw new NotSupportedException(SRFormat.Format(SR.XmlRpcArrayOfValueTypes, elementType.FullName));
                }
    ...
    internal class XmlSerializationReaderCodeGen : XmlSerializationCodeGen
    {
        ...
        private string GetChoiceIdentifierSource(MemberMapping[] mappings, MemberMapping member)
        {
            ...
            if (member.ChoiceIdentifier != null)
            {
                for (int j = 0; j < mappings.Length; j++)
                {
                    ...
                    if (choiceSource == null) throw new InvalidOperationException(SRFormat.Format(SR.XmlInternalErrorDetails, "Can not find " + member.ChoiceIdentifier.MemberName + " in the members mapping."));
#endif
        ...
        private void WritePrimitive(TypeMapping mapping, string source)
        {
            if (mapping is EnumMapping)
            {
                ...
                if (enumMethodName == null) throw new InvalidOperationException(SRFormat.Format(SR.XmlMissingMethodEnum, mapping.TypeDesc.Name));
                if (mapping.IsSoap)
                {
...
#if DEBUG
                    // use exception in the place of Debug.Assert to avoid throwing asserts from a server process such as aspnet_ewp.exe
                    if (methodName == null) throw new InvalidOperationException(SRFormat.Format(SR.XmlInternalErrorMethod, derived.TypeDesc.Name));
#endif
...
#if DEBUG
                            // use exception in the place of Debug.Assert to avoid throwing asserts from a server process such as aspnet_ewp.exe
                            if (methodName == null) throw new InvalidOperationException(SRFormat.Format(SR.XmlInternalErrorMethod, mapping.TypeDesc.Name));
#endif
            ...
            else
            {
                ...
                for (int i = 0; i < mappings.Length; i++)
                {
                    ...
                    else if (mapping.IsParticle && !mapping.IsSequence)
                    {
                        ...
                        throw new InvalidOperationException(SRFormat.Format(SR.XmlSequenceHierarchy, structMapping.TypeDesc.FullName, mapping.Name, declaringMapping.TypeDesc.FullName, "Order"));
                    }
...
#if DEBUG
                // use exception in the place of Debug.Assert to avoid throwing asserts from a server process such as aspnet_ewp.exe
                if (methodName == null) throw new InvalidOperationException(SRFormat.Format(SR.XmlInternalErrorMethod, element.Mapping.TypeDesc.Name));
#endif
                WriteSourceBegin(source);
                Writer.Write(methodName);
                Writer.Write("(true)");
...
#if DEBUG
                    // use exception in the place of Debug.Assert to avoid throwing asserts from a server process such as aspnet_ewp.exe
                    if (choiceSource == null) throw new InvalidOperationException(SRFormat.Format(SR.XmlInternalErrorDetails, "need parent for the " + source));
#endif
