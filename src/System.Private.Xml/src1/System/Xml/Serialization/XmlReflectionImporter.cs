...
namespace SGen.Xml.Serialization
{
    ...
    public class XmlReflectionImporter
    {
        ...
        private void IncludeType(Type type, RecursionLimiter limiter)
        {
            ...
            if (mapping.IsAnonymousType && !mapping.TypeDesc.IsSpecial)
            {
                ...
                throw new InvalidOperationException(SRFormat.Format(SR.XmlAnonymousInclude, type.FullName));
            }
        ...
        private ElementAccessor ImportElement(TypeModel model, XmlRootAttribute root, string defaultNamespace, RecursionLimiter limiter)
        {
            ...
            if (root != null)
            {
                ...
                if (root.IsNullableSpecified && !root.IsNullable && model.TypeDesc.IsOptionalValue)
                    ...
                    throw new InvalidOperationException(SRFormat.Format(SR.XmlInvalidNotNullable, model.TypeDesc.BaseTypeDesc.FullName, "XmlRoot"));
        ...
        private Accessor ReconcileAccessor(Accessor accessor, NameTable accessors)
        {
            ...
            if (!(accessor.Mapping is MembersMapping) && !(existing.Mapping is MembersMapping))
            {
                    ...
                    || (accessor.Mapping is NullableMapping && ((NullableMapping)accessor.Mapping).BaseMapping.TypeDesc == existing.Mapping.TypeDesc))
                {
                    ...
                    throw new InvalidOperationException(SRFormat.Format(SR.XmlCannotReconcileAccessorDefault, accessor.Name, accessor.Namespace, value1, value2));
                }
            }

            if (accessor.Mapping is MembersMapping || existing.Mapping is MembersMapping)
                throw new InvalidOperationException(SRFormat.Format(SR.XmlMethodTypeNameConflict, accessor.Name, accessor.Namespace));

            if (accessor.Mapping is ArrayMapping)
            {
                if (!(existing.Mapping is ArrayMapping))
                {
                    throw new InvalidOperationException(SRFormat.Format(SR.XmlCannotReconcileAccessor, accessor.Name, accessor.Namespace, GetMappingName(existing.Mapping), GetMappingName(accessor.Mapping)));
                }
            ...
            if (accessor is AttributeAccessor)
                throw new InvalidOperationException(SRFormat.Format(SR.XmlCannotReconcileAttributeAccessor, accessor.Name, accessor.Namespace, GetMappingName(existing.Mapping), GetMappingName(accessor.Mapping)));
            else
                throw new InvalidOperationException(SRFormat.Format(SR.XmlCannotReconcileAccessor, accessor.Name, accessor.Namespace, GetMappingName(existing.Mapping), GetMappingName(accessor.Mapping)));
        }
        ...
        {
            return new InvalidOperationException(SRFormat.Format(SR.XmlReflectionError, context), e);
        }

        private Exception CreateTypeReflectionException(string context, Exception e)
        {
            return new InvalidOperationException(SRFormat.Format(SR.XmlTypeReflectionError, context), e);
        }

        private Exception CreateMemberReflectionException(FieldModel model, Exception e)
        {
            return new InvalidOperationException(SRFormat.Format(model.IsProperty ? SR.XmlPropertyReflectionError : SR.XmlFieldReflectionError, model.Name), e);
        }
        ...
        private TypeMapping ImportTypeMapping(TypeModel model, string ns, ImportContext context, string dataType, XmlAttributes a, bool repeats, bool openModel, RecursionLimiter limiter)
        {
            try
            {
                if (dataType.Length > 0)
                {
                    ...
                    {
                        throw new InvalidOperationException(SRFormat.Format(SR.XmlInvalidDataTypeUsage, dataType, "XmlElementAttribute.DataType"));
                    }
                    TypeDesc td = _typeScope.GetTypeDesc(dataType, XmlSchema.Namespace);
                    if (td == null)
                    {
                        throw new InvalidOperationException(SRFormat.Format(SR.XmlInvalidXsdDataType, dataType, "XmlElementAttribute.DataType", new XmlQualifiedName(dataType, XmlSchema.Namespace).ToString()));
                    }
                    if (modelTypeDesc.FullName != td.FullName)
                    {
                        throw new InvalidOperationException(SRFormat.Format(SR.XmlDataTypeMismatch, dataType, "XmlElementAttribute.DataType", modelTypeDesc.FullName));
                    }
                }
                ...
                if ((a.XmlFlags & ~(XmlAttributeFlags.Type | XmlAttributeFlags.Root)) != 0)
                    throw new InvalidOperationException(SRFormat.Format(SR.XmlInvalidTypeAttributes, model.Type.FullName));
                ...
                switch (model.TypeDesc.Kind)
                {
                    ...
                    default:
                        if (model.TypeDesc.Kind == TypeKind.Serializable)
                        {
                            ...
                            {
                                throw new InvalidOperationException(SRFormat.Format(SR.XmlSerializableAttributes, model.TypeDesc.FullName, typeof(XmlSchemaProviderAttribute).Name));
                            }
                        }
                        else
                        {
        ...
        internal static MethodInfo GetMethodFromSchemaProvider(XmlSchemaProviderAttribute provider, Type type)
        {
            ...
            if (!CSharpHelpers.IsValidLanguageIndependentIdentifier(provider.MethodName))
                throw new ArgumentException(SRFormat.Format(SR.XmlGetSchemaMethodName, provider.MethodName), nameof(provider.MethodName));
            ...
            if (getMethod == null)
                throw new InvalidOperationException(SRFormat.Format(SR.XmlGetSchemaMethodMissing, provider.MethodName, typeof(XmlSchemaSet).Name, type.FullName));
            ...
            if (!(typeof(XmlQualifiedName).IsAssignableFrom(getMethod.ReturnType)) && !(typeof(XmlSchemaType).IsAssignableFrom(getMethod.ReturnType)))
                throw new InvalidOperationException(SRFormat.Format(SR.XmlGetSchemaMethodReturnType, type.Name, provider.MethodName, typeof(XmlSchemaProviderAttribute).Name, typeof(XmlQualifiedName).FullName, typeof(XmlSchemaType).FullName));
        ...
        internal static void ValidationCallbackWithErrorCode(object sender, ValidationEventArgs args)
        {
            ...
            if (args.Severity == XmlSeverityType.Error)
                throw new InvalidOperationException(SRFormat.Format(SR.XmlSerializableSchemaError, typeof(IXmlSerializable).Name, args.Message));
        }
        ...
        //    {
        //        throw new InvalidOperationException(SRFormat.Format(SR.XmlMissingSchema, baseQname.Namespace));
        //    }
        //    if (srcSchemas.Count > 1)
        //    {
        //        throw new InvalidOperationException(SRFormat.Format(SR.XmlGetSchemaInclude, baseQname.Namespace, typeof(IXmlSerializable).Name, "GetSchema"));
        //    }
        ...
        {
            return new InvalidOperationException(SRFormat.Format(SR.XmlInvalidAttributeUse, type.FullName));
        }

        private static Exception UnsupportedException(TypeDesc typeDesc, ImportContext context)
        {
            return new InvalidOperationException(SRFormat.Format(SR.XmlIllegalTypeContext, typeDesc.FullName, GetContextName(context)));
        }
        ...
        private NullableMapping CreateNullableMapping(TypeMapping baseMapping, Type type)
        {
            ...
            if (existingMapping != null)
            {
                if (existingMapping is NullableMapping)
                {
                    ...
                    {
                        throw new InvalidOperationException(SRFormat.Format(SR.XmlTypesDuplicate, typeDesc.FullName, existingMapping.TypeDesc.FullName, typeDesc.Name, existingMapping.Namespace));
                    }
                }
                else
                {
        ...
        private TypeMapping GetTypeMapping(string typeName, string ns, TypeDesc typeDesc, NameTable typeLib, Type type)
        {
            ...
            if (!mapping.IsAnonymousType && mapping.TypeDesc != typeDesc)
                throw new InvalidOperationException(SRFormat.Format(SR.XmlTypesDuplicate, typeDesc.FullName, mapping.TypeDesc.FullName, typeName, ns));
            return mapping;
        }
            ...
            if (mapping == null)
            {
                ...
                while (limiter.DeferredWorkItems.Count > 0)
                {
                    ...
                    if (index != limiter.DeferredWorkItems.Count - 1)
                        throw new InvalidOperationException(SRFormat.Format(SR.XmlInternalErrorDetails, "DeferredWorkItems.Count have changed"));
                    if (item != limiter.DeferredWorkItems[index])
                        throw new InvalidOperationException(SRFormat.Format(SR.XmlInternalErrorDetails, "DeferredWorkItems.Top have changed"));
#endif
        ...
        private bool InitializeStructMembers(StructMapping mapping, StructModel model, bool openModel, string typeName, RecursionLimiter limiter)
        {
            ...
            if (model.TypeDesc.BaseTypeDesc != null)
            {
                ...
                if (!(baseModel is StructModel))
                {
                    ...
                    throw new NotSupportedException(SRFormat.Format(SR.XmlUnsupportedInheritance, model.Type.BaseType.FullName));
                }
                ...
                try
                {
                    ...
                    if (member.Text != null)
                    {
                        if (!member.Text.Mapping.TypeDesc.CanBeTextValue && member.Text.Mapping.IsList)
                            throw new InvalidOperationException(SRFormat.Format(SR.XmlIllegalTypedTextAttribute, typeName, member.Text.Name, member.Text.Mapping.TypeDesc.FullName));
                        if (textAccesor != null)
                        {
                            throw new InvalidOperationException(SRFormat.Format(SR.XmlIllegalMultipleText, model.Type.FullName));
                        }
                    ...
                    if (member.Xmlns != null)
                    {
                        if (mapping.XmlnsMember != null)
                            throw new InvalidOperationException(SRFormat.Format(SR.XmlMultipleXmlns, model.Type.FullName));
            ...
            if (isSequence)
            {
                ...
                for (int i = 0; i < members.Count; i++)
                {
                    ...
                    if (member.IsSequence)
                    {
                        ...
                        {
                            throw new InvalidOperationException(SRFormat.Format(SR.XmlSequenceUnique, member.SequenceId.ToString(CultureInfo.InvariantCulture), "Order", member.Name));
                        }
                        ids[member.SequenceId] = member;
                    }
                    else
                    {
                        throw new InvalidOperationException(SRFormat.Format(SR.XmlSequenceInconsistent, "Order", member.Name));
                    }
                }
        ...
        private PrimitiveMapping ImportPrimitiveMapping(PrimitiveModel model, ImportContext context, string dataType, bool repeats)
        {
            ...
            if (dataType.Length > 0)
            {
                ...
                if (mapping.TypeDesc == null)
                {
                    ...
                    if (mapping.TypeDesc == null)
                    {
                        throw new InvalidOperationException(SRFormat.Format(SR.XmlUdeclaredXsdType, dataType));
                    }
                }
            }
            else
        ...
        private EnumMapping ImportEnumMapping(EnumModel model, string ns, bool repeats)
        {
            ...
            if (mapping == null)
            {
                ...
                if (mapping.IsFlags && repeats)
                    throw new InvalidOperationException(SRFormat.Format(SR.XmlIllegalAttributeFlagsArray, model.TypeDesc.FullName));
                mapping.IsList = repeats;
                ...
                {
                    throw new InvalidOperationException(SRFormat.Format(SR.XmlNoSerializableMembers, model.TypeDesc.FullName));
                }
        ...
        private MembersMapping ImportMembersMapping(XmlReflectionMember[] xmlReflectionMembers, string ns, bool hasWrapperElement, bool rpc, bool openModel, RecursionLimiter limiter)
        {
                ...
                try
                {
                    ...
                    if (!hasWrapperElement)
                    {
                        if (mapping.Attribute != null)
                        {
                            ...
                            {
                                throw new InvalidOperationException(SRFormat.Format(SR.XmlInvalidAttributeType, "XmlAttribute"));
                            }
                        }
                    }
            ...
            if (isSequence)
            {
                throw new InvalidOperationException(SRFormat.Format(SR.XmlSequenceMembers, "Order"));
            }
        ...
        private Type CheckChoiceIdentifierType(Type type, bool isArrayLike, string identifierName, string memberName)
        {
            if (type.IsArray)
            {
                if (!isArrayLike)
                {
                    ...
                    throw new InvalidOperationException(SRFormat.Format(SR.XmlChoiceIdentifierType, identifierName, memberName, type.GetElementType().FullName));
                }
            ...
            {
                // Inconsistent type of the choice identifier '{0}'.  Please use {1}.
                throw new InvalidOperationException(SRFormat.Format(SR.XmlChoiceIdentifierArrayType, identifierName, memberName, type.FullName));
            }

            if (!type.IsEnum)
            {
                // Choice identifier '{0}' must be an enum.
                throw new InvalidOperationException(SRFormat.Format(SR.XmlChoiceIdentifierTypeEnum, identifierName));
            }
            return type;
        }
        ...
        private Type GetChoiceIdentifierType(XmlChoiceIdentifierAttribute choice, XmlReflectionMember[] xmlReflectionMembers, bool isArrayLike, string accessorName)
        {
            ...
            throw new InvalidOperationException(SRFormat.Format(SR.XmlChoiceIdentiferMemberMissing, choice.MemberName, accessorName));
        }
        ...
        private Type GetChoiceIdentifierType(XmlChoiceIdentifierAttribute choice, StructModel structModel, bool isArrayLike, string accessorName)
        {
            ...
            else if (infos.Length > 1)
            {
                ...
                throw new InvalidOperationException(SRFormat.Format(SR.XmlChoiceIdentiferAmbiguous, choice.MemberName));
            }
        ...
        private void ImportAccessorMapping(MemberMapping accessor, FieldModel model, XmlAttributes a, string ns, Type choiceIdentifierType, bool rpc, bool openModel, RecursionLimiter limiter)
        {
            ...
            if (accessor.TypeDesc.IsArrayLike)
            {
                ...
                if ((flags & attrFlags) != 0)
                {
                    ...
                    if (a.XmlAttribute != null && !accessor.TypeDesc.ArrayElementTypeDesc.IsPrimitive && !accessor.TypeDesc.ArrayElementTypeDesc.IsEnum)
                    {
                        ...
                        {
                            throw new InvalidOperationException(SRFormat.Format(SR.XmlIllegalAttrOrTextInterface, accessorName, accessor.TypeDesc.ArrayElementTypeDesc.FullName, typeof(IXmlSerializable).Name));
                        }
                        else
                        {
                            throw new InvalidOperationException(SRFormat.Format(SR.XmlIllegalAttrOrText, accessorName, accessor.TypeDesc.ArrayElementTypeDesc.FullName));
                        }
                    }
                ...
                else if ((flags & elemFlags) != 0)
                {
                    ...
                    if (a.XmlText != null)
                    {
                        ...
                        if (!(text.Mapping is SpecialMapping) && targetTypeDesc != _typeScope.GetTypeDesc(typeof(string)))
                            throw new InvalidOperationException(SRFormat.Format(SR.XmlIllegalArrayTextAttribute, accessorName));
                    ...
                    for (int i = 0; i < a.XmlElements.Count; i++)
                    {
                        ...
                        if (xmlElement.IsNullableSpecified && !xmlElement.IsNullable && typeModel.TypeDesc.IsOptionalValue)
                            //XmlInvalidNotNullable=IsNullable may not be set to 'false' for a Nullable<{0}> type. Consider using '{0}' type or removing the IsNullable property from the XmlElement attribute.
                            throw new InvalidOperationException(SRFormat.Format(SR.XmlInvalidNotNullable, typeModel.TypeDesc.BaseTypeDesc.FullName, "XmlElement"));
                        ...
                        if (xmlElement.Order != -1)
                        {
                            if (xmlElement.Order != sequenceId && sequenceId != -1)
                                throw new InvalidOperationException(SRFormat.Format(SR.XmlSequenceMatch, "Order"));
                    ...
                    for (int i = 0; i < a.XmlAnyElements.Count; i++)
                    {
                        ...
                        if (!arrayElementType.IsAssignableFrom(targetType))
                            throw new InvalidOperationException(SRFormat.Format(SR.XmlIllegalAnyElement, arrayElementType.FullName));
                        ...
                        {
                            throw new InvalidOperationException(SRFormat.Format(SR.XmlAnyElementDuplicate, accessorName, xmlAnyElement.Name, xmlAnyElement.Namespace == null ? "null" : xmlAnyElement.Namespace));
                        }
                        ElementAccessor element = new ElementAccessor();
            ...
            else if (!accessor.TypeDesc.IsVoid)
            {
                ...
                {
                    if (a.XmlAnyElements.Count > 0) throw new InvalidOperationException(SRFormat.Format(SR.XmlIllegalAnyElement, accessor.TypeDesc.FullName));

                    if (a.XmlAttribute != null)
                    {
                        if (a.XmlElements.Count > 0) throw new InvalidOperationException(SR.XmlIllegalAttribute);
                        if (a.XmlAttribute.Type != null) throw new InvalidOperationException(SRFormat.Format(SR.XmlIllegalType, "XmlAttribute"));
                        AttributeAccessor attribute = new AttributeAccessor();
                        attribute.Name = Accessor.EscapeQName(a.XmlAttribute.AttributeName.Length == 0 ? accessorName : a.XmlAttribute.AttributeName);
                        attribute.Namespace = a.XmlAttribute.Namespace == null ? ns : a.XmlAttribute.Namespace;
                        attribute.Form = a.XmlAttribute.Form;
                    ...
                    else
                    {
                        if (a.XmlText != null)
                        {
                            if (a.XmlText.Type != null && a.XmlText.Type != accessorType)
                                throw new InvalidOperationException(SRFormat.Format(SR.XmlIllegalType, "XmlText"));
                            TextAccessor text = new TextAccessor();
                            text.Name = accessorName; // unused except to make more helpful error messages
                        ...
                        for (int i = 0; i < a.XmlElements.Count; i++)
                        {
                            ...
                            if (xmlElement.Type != null)
                            {
                                if (_typeScope.GetTypeDesc(xmlElement.Type) != accessor.TypeDesc)
                                    throw new InvalidOperationException(SRFormat.Format(SR.XmlIllegalType, "XmlElement"));
                            }
                            ElementAccessor element = new ElementAccessor();
                            element.Name = XmlConvert.EncodeLocalName(xmlElement.ElementName.Length == 0 ? accessorName : xmlElement.ElementName);
                            element.Namespace = rpc ? null : xmlElement.Namespace == null ? ns : xmlElement.Namespace;
                ...
                else if (a.Xmlns)
                {
                    ...
                    {
                        throw new InvalidOperationException(SRFormat.Format(SR.XmlXmlnsInvalidType, accessorName, accessorType.FullName, typeof(XmlSerializerNamespaces).FullName));
                    }
                ...
                else
                {
                    if (a.XmlAttribute != null || a.XmlText != null)
                    {
                        ...
                        {
                            throw new InvalidOperationException(SRFormat.Format(SR.XmlIllegalAttrOrTextInterface, accessorName, accessor.TypeDesc.FullName, typeof(IXmlSerializable).Name));
                        }
                        else
                        {
                            throw new InvalidOperationException(SRFormat.Format(SR.XmlIllegalAttrOrText, accessorName, accessor.TypeDesc));
                        }
                    }
                    ...
                    for (int i = 0; i < a.XmlAnyElements.Count; i++)
                    {
                        ...
                        if (!accessorType.IsAssignableFrom(targetType))
                            throw new InvalidOperationException(SRFormat.Format(SR.XmlIllegalAnyElement, accessorType.FullName));
            ...
            if (rpc)
            {
                if (accessor.TypeDesc.IsArrayLike && accessor.Elements.Length > 0 && !(accessor.Elements[0].Mapping is ArrayMapping))
                    throw new InvalidOperationException(SRFormat.Format(SR.XmlRpcLitArrayElement, accessor.Elements[0].Name));

                if (accessor.Xmlns != null)
                    throw new InvalidOperationException(SRFormat.Format(SR.XmlRpcLitXmlns, accessor.Name));
            }
                ...
                for (int i = 0; i < accessor.Elements.Length; i++)
                {
                    ...
                    if (!found)
                    {
                        if (element.Any && element.Name.Length == 0)
                        {
                            ...
                            throw new InvalidOperationException(SRFormat.Format(SR.XmlChoiceMissingAnyValue, accessor.ChoiceIdentifier.Mapping.TypeDesc.FullName));
                        }
                        else
                        {
                            ...
                            throw new InvalidOperationException(SRFormat.Format(SR.XmlChoiceMissingValue, accessor.ChoiceIdentifier.Mapping.TypeDesc.FullName, id, element.Name, element.Namespace));
                        }
                    }
                }
            }
        ...
        private void CheckTopLevelAttributes(XmlAttributes a, string accessorName)
        {
            ...
            if (a.XmlElements != null && a.XmlElements.Count > 0)
            {
                ...
                {
                    throw new InvalidOperationException(SRFormat.Format(SR.XmlRpcLitElementNamespace, "Namespace", xmlElement.Namespace));
                }
                if (xmlElement.IsNullable)
                {
                    throw new InvalidOperationException(SRFormat.Format(SR.XmlRpcLitElementNullable, "IsNullable", "true"));
                }
            }
            if (a.XmlArray != null && a.XmlArray.Namespace != null)
            {
                throw new InvalidOperationException(SRFormat.Format(SR.XmlRpcLitElementNamespace, "Namespace", a.XmlArray.Namespace));
            }
        }
        ...
        private void CheckAmbiguousChoice(XmlAttributes a, Type accessorType, string accessorName)
        {
            ...
            if (elements != null && elements.Count >= 2 && a.XmlChoiceIdentifier == null)
            {
                for (int i = 0; i < elements.Count; i++)
                {
                    ...
                    if (choiceTypes.Contains(type))
                    {
                        ...
                        throw new InvalidOperationException(SRFormat.Format(SR.XmlChoiceIdentiferMissing, typeof(XmlChoiceIdentifierAttribute).Name, accessorName));
                    }
                    else
                    {
            ...
            if (items != null && items.Count >= 2)
            {
                ...
                for (int i = 0; i < items.Count; i++)
                {
                    ...
                    if (item != null)
                    {
                        throw new InvalidOperationException(SRFormat.Format(SR.XmlArrayItemAmbiguousTypes, accessorName, item.ElementName, items[i].ElementName, typeof(XmlElementAttribute).Name, typeof(XmlChoiceIdentifierAttribute).Name, accessorName));
                    }
                    else
                    {
        ...
        private void CheckChoiceIdentifierMapping(EnumMapping choiceMapping)
        {
            ...
            for (int i = 0; i < choiceMapping.Constants.Length; i++)
            {
                ...
                if (ids[choiceName, choiceNs] != null)
                {
                    ...
                    throw new InvalidOperationException(SRFormat.Format(SR.XmlChoiceIdDuplicate, choiceMapping.TypeName, choiceId));
                }
        ...
        private object GetDefaultValue(TypeDesc fieldTypeDesc, Type t, XmlAttributes a)
        {
            ...
            if (fieldTypeDesc.Kind == TypeKind.Enum)
            {
                ...
                if (strValue == numValue) // means enum value wasn't recognized
                    throw new InvalidOperationException(SRFormat.Format(SR.XmlInvalidDefaultValue, strValue, a.XmlDefaultValue.GetType().FullName));
                return strValue;
            }
            return a.XmlDefaultValue;
        }
        ...
        private static void AddUniqueAccessor(INameScope scope, Accessor accessor)
        {
            ...
            if (existing != null)
            {
                ...
                {
                    throw new InvalidOperationException(SRFormat.Format(SR.XmlDuplicateElementName, existing.Name, existing.Namespace));
                }
                else
                {
#if DEBUG
                    if (!(accessor is AttributeAccessor))
                        throw new InvalidOperationException(SRFormat.Format(SR.XmlInternalErrorDetails, "Bad accessor type " + accessor.GetType().FullName));
#endif
                    throw new InvalidOperationException(SRFormat.Format(SR.XmlDuplicateAttributeName, existing.Name, existing.Namespace));
                }
            }
            else
            {
        ...
        private static void CheckNullable(bool isNullable, TypeDesc typeDesc, TypeMapping mapping)
        {
            ...
            if (isNullable && !typeDesc.IsNullable) throw new InvalidOperationException(SRFormat.Format(SR.XmlInvalidIsNullable, typeDesc.FullName));
        }
