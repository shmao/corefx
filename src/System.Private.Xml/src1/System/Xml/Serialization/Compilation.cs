...
namespace SGen.Xml.Serialization
{
    ...
    internal class TempAssembly
    {
        ...
        internal TempAssembly(XmlMapping[] xmlMappings, Type[] types, string defaultNamespace, string location)
        {
            ...
            if (_assembly == null) throw new InvalidOperationException(SRFormat.Format(SR.XmlInternalErrorDetails, "Failed to generate XmlSerializer assembly, but did not throw"));
#endif
...
#pragma warning restore 618
            ...
            else
            {
                ...
                if (assemblyAttribute.AssemblyName != null && assemblyAttribute.CodeBase != null)
                    throw new InvalidOperationException(SRFormat.Format(SR.XmlPregenInvalidXmlSerializerAssemblyAttribute, "AssemblyName", "CodeBase"));
...
#pragma warning restore 618
        ...
        internal static Type GetTypeFromAssembly(Assembly assembly, string typeName)
        {
            ...
            if (type == null) throw new InvalidOperationException(SRFormat.Format(SR.XmlMissingType, typeName, assembly.FullName));
            return type;
        }
        ...
        private string ValidateEncodingStyle(string encodingStyle, string methodKey)
        {
            if (encodingStyle != null && encodingStyle.Length > 0)
            {
                if (_methods[methodKey].isSoap)
                {
                    ...
                    {
                        throw new InvalidOperationException(SRFormat.Format(SR.XmlInvalidEncoding3, encodingStyle, Soap.Encoding, Soap12.Encoding));
                    }
                }
                else
                {
                    throw new InvalidOperationException(SRFormat.Format(SR.XmlInvalidEncodingNotEncoded1, encodingStyle));
                }
            }
            else
            {
        ...
        internal object InvokeReader(XmlMapping mapping, XmlReader xmlReader, XmlDeserializationEvents events, string encodingStyle)
        {
            ...
            try
            {
                ...
                if (_methods[mapping.Key].readMethod == null)
                {
                    ...
                    if (methodName == null)
                    {
                        throw new InvalidOperationException(SRFormat.Format(SR.XmlNotSerializable, mapping.Accessor.Name));
                    }
