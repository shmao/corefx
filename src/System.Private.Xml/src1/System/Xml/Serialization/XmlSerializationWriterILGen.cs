...
#if DEBUG
                // use exception in the place of Debug.Assert to avoid throwing asserts from a server process such as aspnet_ewp.exe
                if (methodName == null) throw new InvalidOperationException("derived from " + mapping.TypeDesc.FullName + ", " + SRFormat.Format(SR.XmlInternalErrorMethod, derived.TypeDesc.Name));
#endif
