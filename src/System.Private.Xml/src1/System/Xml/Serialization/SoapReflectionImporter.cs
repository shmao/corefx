    ...
    public class SoapReflectionImporter
    {
        ...
        private TypeMapping ImportTypeMapping(TypeModel model, string dataType, RecursionLimiter limiter)
        {
            if (dataType.Length > 0)
            {
                ...
                {
                    throw new InvalidOperationException(SRFormat.Format(SR.XmlInvalidDataTypeUsage, dataType, "SoapElementAttribute.DataType"));
                }
                TypeDesc td = _typeScope.GetTypeDesc(dataType, XmlSchema.Namespace);
                if (td == null)
                {
                    throw new InvalidOperationException(SRFormat.Format(SR.XmlInvalidXsdDataType, dataType, "SoapElementAttribute.DataType", new XmlQualifiedName(dataType, XmlSchema.Namespace).ToString()));
                }
                if (model.TypeDesc.FullName != td.FullName)
                {
                    throw new InvalidOperationException(SRFormat.Format(SR.XmlDataTypeMismatch, dataType, "SoapElementAttribute.DataType", model.TypeDesc.FullName));
                }
            }
            ...
            switch (model.TypeDesc.Kind)
            {
                ...
                default:
                    throw new NotSupportedException(SRFormat.Format(SR.XmlUnsupportedSoapTypeKind, model.TypeDesc.FullName));
            }
        }
            ...
            foreach (MemberInfo memberInfo in model.GetMemberInfos())
            {
                ...
                if (!member.TypeDesc.IsPrimitive && !member.TypeDesc.IsEnum && !member.TypeDesc.IsOptionalValue)
                {
                    if (model.TypeDesc.IsValueType)
                        throw new NotSupportedException(SRFormat.Format(SR.XmlRpcRefsInValueType, model.TypeDesc.FullName));
                    if (member.TypeDesc.IsValueType)
                        throw new NotSupportedException(SRFormat.Format(SR.XmlRpcNestedValueType, member.TypeDesc.FullName));
                }
        ...
        private ArrayMapping ImportArrayLikeMapping(ArrayModel model, RecursionLimiter limiter)
        {
            ...
            if (itemTypeMapping.TypeDesc.IsValueType && !itemTypeMapping.TypeDesc.IsPrimitive && !itemTypeMapping.TypeDesc.IsEnum)
                throw new NotSupportedException(SRFormat.Format(SR.XmlRpcArrayOfValueTypes, model.TypeDesc.FullName));
        ...
        private void SetArrayMappingType(ArrayMapping mapping)
        {
            ...
            {
                throw new InvalidOperationException(SRFormat.Format(SR.XmlInvalidSoapArray, mapping.TypeDesc.FullName));
            }
        ...
        private void ImportAccessorMapping(MemberMapping accessor, FieldModel model, SoapAttributes a, string ns, XmlSchemaForm form, RecursionLimiter limiter)
        {
            ...
            if ((flags & SoapAttributeFlags.Attribute) == SoapAttributeFlags.Attribute)
            {
                if (!accessor.TypeDesc.IsPrimitive && !accessor.TypeDesc.IsEnum)
                    throw new InvalidOperationException(SRFormat.Format(SR.XmlIllegalSoapAttribute, accessorName, accessor.TypeDesc.FullName));
        ...
        private object GetDefaultValue(TypeDesc fieldTypeDesc, SoapAttributes a)
        {
            ...
            if (fieldTypeDesc.Kind == TypeKind.Enum)
            {
                if (fieldTypeDesc != _typeScope.GetTypeDesc(a.SoapDefaultValue.GetType()))
                    throw new InvalidOperationException(SRFormat.Format(SR.XmlInvalidDefaultEnumValue, a.SoapDefaultValue.GetType().FullName, fieldTypeDesc.FullName));
                ...
                if (strValue == numValue) // means enum value wasn't recognized
                    throw new InvalidOperationException(SRFormat.Format(SR.XmlInvalidDefaultValue, strValue, a.SoapDefaultValue.GetType().FullName));
                return strValue;
            }
            return a.SoapDefaultValue;
        }
