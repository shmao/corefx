...
namespace SGen.Xml.Serialization
{
    ...
    public class CodeIdentifier
    {
        ...
        private static bool IsValidStart(char c)
        {
            ...
            if (!IsValid(c)) throw new ArgumentException(SRFormat.Format(SR.XmlInternalErrorDetails, "Invalid identifier character " + ((Int16)c).ToString(CultureInfo.InvariantCulture)), "c");
#endif
...
#if DEBUG
                    // use exception in the place of Debug.Assert to avoid throwing asserts from a server process such as aspnet_ewp.exe
                    throw new ArgumentException(SRFormat.Format(SR.XmlInternalErrorDetails, "Unhandled category " + uc), "c");
#else
                return false;
#endif
            }
        ...
        internal static void CheckValidIdentifier(string ident)
        {
            if (!CSharpHelpers.IsValidLanguageIndependentIdentifier(ident))
                throw new ArgumentException(SRFormat.Format(SR.XmlInvalidIdentifier, ident), nameof(ident));
        }
