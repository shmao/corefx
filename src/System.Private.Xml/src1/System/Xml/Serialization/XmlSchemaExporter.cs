            ...
            else
            {
                foreach (MemberMapping member in mapping.Members)
                {
                    if (member.Attribute != null)
                        throw new InvalidOperationException(SRFormat.Format(SR.XmlBareAttributeMember, member.Attribute.Name));
                    else if (member.Text != null)
                        throw new InvalidOperationException(SRFormat.Format(SR.XmlBareTextMember, member.Text.Name));
                    ...
                    if (member.TypeDesc.IsArrayLike && !(member.Elements[0].Mapping is ArrayMapping))
                        throw new InvalidOperationException(SRFormat.Format(SR.XmlIllegalArrayElement, member.Elements[0].Name));
            ...
            if (schema != null)
            {
                foreach (XmlSchemaObject o in schema.Items)
                {
                    ...
                    if (type != null && type.Name == newTypeName)
                        throw new InvalidOperationException(SRFormat.Format(SR.XmlDuplicateTypeName, newTypeName, newNamespace));
                }
            }
        }
...
#pragma warning restore 429
        ...
        private void AddSchemaItem(XmlSchemaObject item, string ns, string referencingNs)
        {
            ...
            if (item is XmlSchemaElement)
            {
                ...
                if (e.Form == XmlSchemaForm.Unqualified)
                    throw new InvalidOperationException(SRFormat.Format(SR.XmlIllegalForm, e.Name));
            ...
            else if (item is XmlSchemaAttribute)
            {
                ...
                if (a.Form == XmlSchemaForm.Unqualified)
                    throw new InvalidOperationException(SRFormat.Format(SR.XmlIllegalForm, a.Name));
                ...
                case TypeKind.Serializable:
                    {
                        ...
                        else if (serializableMapping.Schema != null)
                        {
                            ...
                            {
                                throw new InvalidOperationException(SRFormat.Format(SR.XmlDuplicateNamespace, anyNs));
                            }
                            if (element != null)
                                element.SchemaType = type;
        ...
        private XmlSchemaType ExportAnonymousPrimitiveMapping(PrimitiveMapping mapping)
        {
            ...
            {
                throw new InvalidOperationException(SRFormat.Format(SR.XmlInternalErrorDetails, "Unsupported anonymous mapping type: " + mapping.ToString()));
            }
        }
            ...
            if (type == null)
            {
                ...
                if (seq.Items.Count > 0)
                {
                        ...
                        if (seq.Items.Count != 1) 
                            throw new InvalidOperationException(SRFormat.Format(SR.XmlInternalErrorDetails, "Type " + mapping.TypeName + " from namespace '" + ns + "' is an invalid array mapping"));
#endif
        ...
        private void ExportAttributeAccessor(XmlSchemaComplexType type, AttributeAccessor accessor, bool valueTypeOptional, string ns)
        {
            ...
            if (type.ContentModel != null)
            {
                if (type.ContentModel.Content is XmlSchemaComplexContentRestriction)
                    ...
                    throw new InvalidOperationException(SRFormat.Format(SR.XmlInvalidContent, type.ContentModel.Content.GetType().Name));
            }
            else
            {
        ...
        internal static string ExportDefaultValue(TypeMapping mapping, object value)
        {
            ...
            if (mapping is EnumMapping)
            {
                EnumMapping em = (EnumMapping)mapping;
                    ...
                    if (value.GetType() != typeof(string)) throw new InvalidOperationException(SRFormat.Format(SR.XmlInternalErrorDetails, SRFormat.Format(SR.XmlInvalidDefaultValue, value.ToString(), value.GetType().FullName)));
#endif
            ...
            if (!pm.TypeDesc.HasCustomFormatter)
            {
                    ...
                    if (pm.TypeDesc.Type == null) {
                        throw new InvalidOperationException(SRFormat.Format(SR.XmlInternalErrorDetails, "Mapping for " + pm.TypeDesc.Name + " missing type property"));
                    }
#endif
            ...
            else
            {
                ...
                if (defaultValue == null)
                    throw new InvalidOperationException(SRFormat.Format(SR.XmlInvalidDefaultValue, value.ToString(), pm.TypeDesc.Name));
                return defaultValue;
            }
            throw new InvalidOperationException(SRFormat.Format(SR.XmlInvalidDefaultValue, value.ToString(), pm.TypeDesc.Name));
        }
        ...
        private XmlQualifiedName ExportStructMapping(StructMapping mapping, string ns, XmlSchemaElement element)
        {
            ...
            if (mapping.IsAnonymousType)
            {
                if (_references[mapping] != null)
                    throw new InvalidOperationException(SRFormat.Format(SR.XmlCircularReference2, mapping.TypeDesc.Name, "AnonymousType", "false"));
                _references[mapping] = mapping;
            }
            XmlSchemaComplexType type = (XmlSchemaComplexType)_types[mapping];
            if (type == null)
            {
                if (!mapping.IncludeInSchema) throw new InvalidOperationException(SRFormat.Format(SR.XmlCannotIncludeInSchema, mapping.TypeDesc.Name));
                ...
                if (mapping.BaseMapping != null && mapping.BaseMapping.IncludeInSchema)
                {
                    ...
                    {
                        throw new InvalidOperationException(SRFormat.Format(SR.XmlAnonymousBaseType, mapping.TypeDesc.Name, mapping.BaseMapping.TypeDesc.Name, "AnonymousType", "false"));
                    }
        ...
        private void ExportTypeMembers(XmlSchemaComplexType type, MemberMapping[] members, string name, string ns, bool hasSimpleContent, bool openModel)
        {
            ...
            for (int i = 0; i < members.Length; i++)
            {
                ...
                if (member.Text != null)
                {
                    if (textMapping != null)
                    {
                        throw new InvalidOperationException(SRFormat.Format(SR.XmlIllegalMultipleText, name));
                    }
                ...
                if (hasSimpleContent)
                {
                    if (textMapping is PrimitiveMapping && seq.Items.Count == 0)
                    {
                        ...
                        else
                        {
                            if (pm.IsAnonymousType)
                            {
                                throw new InvalidOperationException(SRFormat.Format(SR.XmlAnonymousBaseType, textMapping.TypeDesc.Name, pm.TypeDesc.Name, "AnonymousType", "false"));
                            }
