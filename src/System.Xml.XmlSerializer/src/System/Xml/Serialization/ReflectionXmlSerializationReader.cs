        ...
        private object WritePrimitive(TypeMapping mapping, Func<string> readFunc)
        {
            ...
            else
            {
                ...
                if (!mapping.TypeDesc.HasCustomFormatter)
                {
                    ...
                    switch (mapping.TypeDesc.FormatterName)
                    {
                        ...
                        default:
                            ...
                            if (method == null)
                            {
                                throw new InvalidOperationException(SR.Format(SR.XmlInternalErrorMethodNotFound, methodName, typeof(XmlConvert)));
                            }

                            retObj = method.Invoke(null, new object[] { value });
                            break;
                    }
                ...
                else
                {
                    ...
                    if (method == null)
                    {
                        throw new InvalidOperationException(SR.Format(SR.XmlInternalErrorMethodNotFound, methodName, typeof(XmlSerializationReader)));
                    }
