...
namespace System.Xml.Serialization
{
    ...
    public class SoapReflectionImporter
    {
        ...
        void IncludeTypes(MemberInfo memberInfo, RecursionLimiter limiter)
        {
            object[] attrs = memberInfo.GetCustomAttributes(typeof(SoapIncludeAttribute), false).ToArray() ;
        ...
        private ConstantMapping ImportConstantMapping(ConstantModel model)
        {
            ...
            if ((a.SoapFlags & ~SoapAttributeFlags.Enum) != 0)
                throw new InvalidOperationException(SR.Format(SR.XmlInvalidEnumAttribute));
            if (a.SoapEnum == null)
            ...
            for (int i = 0; i < mappings.Length; i++)
            {
                try
                {
                    ...
                    { // no special treatment for return values with doc/enc
                        if (i > 0) throw new InvalidOperationException(SR.Format(SR.XmlInvalidReturnPosition));
        ...
        private void ImportAccessorMapping(MemberMapping accessor, FieldModel model, SoapAttributes a, string ns, XmlSchemaForm form, RecursionLimiter limiter)
        {
            ...
            {
                throw new InvalidOperationException(SR.Format(SR.XmlInvalidVoid));
            }
            ...
            if ((flags & SoapAttributeFlags.Attribute) == SoapAttributeFlags.Attribute)
            {
                ...
                if ((flags & SoapAttributeFlags.Attribute) != flags)
                    throw new InvalidOperationException(SR.Format(SR.XmlInvalidElementAttribute));
        ...
        {
            if (a.SoapDefaultValue == null || Globals.IsDBNullValue(a.SoapDefaultValue)) return null;
